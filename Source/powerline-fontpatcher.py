...
parser.add_argument('--source-font', help='source symbol font', metavar='font', dest='source_font', default='{0}/powerline-symbols.sfd'.format(sys.path[0]), type=argparse.FileType('rb'))
  ...
  def patch(self):
    for target_font in self.target_fonts:
      ...
      if self.rename_font:
        ...
        target_font.fontname = fontname + '-Powerline'
      ...
      if extension.lower() not in ['.ttf', '.otf']:
        ...
        extension = '.otf'
      target_font.generate('{0}{1}'.format(target_font.fontname, extension))
